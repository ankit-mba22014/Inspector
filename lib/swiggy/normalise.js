 ...
 */
// Mobile UPI methods arrive as bare intent schemes (e.g. "gpay://upi/") with
// no label — map the scheme to the app's display name.
...
export function extractPaymentOptions(raw) {
  ...
  const mobile = (d.platforms?.mobile?.methods || []).map((m) => ({
    id: m.id,
    label: m.label || m.name || intentAppLabel(m.id),
    kind: 'upi-intent',
    surface: 'mobile',
  }));
