...
function scoreCandidate(item, query) {
  ...
  // A multi-word query (e.g. a translated "Yellow Mustard Seeds") rarely
  // appears in a catalogue name verbatim or in the same word order
  // ("Supreme Harvest Mustard Yellow") — whole-substring matching alone
  // would score a genuinely matching product as if it were unrelated.
  // Fall back to how many of its significant words show up at all.
  const qWords = q.split(/\s+/).filter((w) => w.length > 2);
  if (relevance === 0 && qWords.length > 1) {
    const matched = qWords.filter((w) => n.includes(w));
    relevance += (matched.length / qWords.length) * 40;
  }
...
export function pickBestMatch(raw, query) {
  ...
  // Below 0 isn't necessarily a bad match — it just means our local text
  // scoring found no literal word overlap with the product's own name
  // (e.g. "bhindi" vs a listing named "Ladies Finger"). Swiggy's own search
  // already matched the query to return this candidate at all, and a manual
  // search of the same query shows exactly this result as the top pick — so
  // trust it down to the same noise floor rankProducts uses for the visible
  // list, instead of a stricter cutoff that only auto-add enforced.
  if (scored[0].score <= -40) return null;
 ...
 */
// Fallback only — Swiggy's own `displayName` is the primary source of a
// method's label (see extractPaymentOptions below). This covers the rare
// case displayName is itself missing, keyed by the bare intent scheme
// (e.g. "gpay://upi/" -> "gpay").
const INTENT_APP_LABELS = {
  gpay: 'Google Pay',
  tez: 'Google Pay',
  phonepe: 'PhonePe',
  paytmmp: 'Paytm',
...
function intentAppLabel(id) {
  const scheme = String(id || '').split('://')[0].toLowerCase();
  return INTENT_APP_LABELS[scheme] || id;
}

// Allowlist, not a blocklist — we only carry real icons (components/PaymentIcon.js)
// for these apps, and want the picker capped to them rather than growing
// automatically every time Swiggy adds a new UPI app to the response.
const ALLOWED_INTENT_SCHEMES = new Set(Object.keys(INTENT_APP_LABELS));

function isAllowedIntent(id) {
  const scheme = String(id || '').split('://')[0].toLowerCase();
  return ALLOWED_INTENT_SCHEMES.has(scheme);
}

export function extractPaymentOptions(raw) {
  const d = unwrap(raw);

  const mobile = (d.platforms?.mobile?.methods || [])
    .filter((m) => isAllowedIntent(m.id))
    .map((m) => ({
      id: m.id,
      label: m.displayName || m.label || m.name || intentAppLabel(m.id),
      kind: 'upi-intent',
      surface: 'mobile',
    }));
  ...
  const desktop = (d.platforms?.desktop?.methods || []).map((m) => ({
    id: m.id,
    label: m.displayName || m.label || m.name || 'Scan QR to pay',
    kind: 'upi-qr',
    surface: 'desktop',
  }));
  ...
  const cash = d.cod
    ? [{
        ...
        label: d.cod.displayName || d.cod.label || 'Cash on delivery',
        kind: 'cash',
        surface: 'any',
      }]
    : [];

  // Settles server-side — no device handoff, no UPI-style long poll. See
  // placeOrder in app/cart/page.js.
  const wallet = d.swiggyMoney?.available
    ? [{
        id: d.swiggyMoney.id || 'SwiggyPay',
        label: d.swiggyMoney.displayName || 'Swiggy Money',
        kind: 'wallet',
        surface: 'any',
      }]
    : [];
