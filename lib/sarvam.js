 ...
 */
export async function transcribeAudio(buffer, { mimeType = 'audio/webm', mode = 'codemix', maxAttempts = 3 } = {}) {
  let attempt = 0;
  ...
  while (true) {
    ...
    // codemix (default) is Sarvam's mode for code-switched speech — exactly
    // the Hindi-English mix a grocery request tends to be. 'translate' asks
    // for English output directly instead — see voice-transcribe/route.js.
